...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def do_OCR(self):
        ...
        if address.endswith('.pdf'):
        ...
        elif address.endswith('.jpg') or address.endswith('.JPG') or address.endswith('.jpeg')  or file_name.endswith('.tiff') or file_name.endswith('.tif') or  address.endswith('.JPEG') or address.endswith('.bmp')  or file_name.endswith('.bmp'):
            ...
            doc['total_page'] = 1
        elif address.endswith('.png') or file_name.endswith('.png') or address.endswith('.PNG'):
            print("address**************=",address)
            im = Image.open(address)
            rgb_im = im.convert('RGB')
            change_format=os.path.splitext(address)
            address=str(change_format[0])+str(".jpg")
            print("address**************=",address)
            rgb_im.save(address )
